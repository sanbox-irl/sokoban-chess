...
pub fn serialize_tiles(
    tilemap: &Tilemap,
    uuid: &str,
) -> Result<FragmentedData<Vec<Option<usize>>>, Error> {
    let vec: Vec<Option<usize>> = tilemap
        .tiles
        .iter()
        .map(|mt| mt.map(|mt| mt.into()))
        .collect();
...
pub fn load_tiles(
    tile_fraged_data: &FragmentedData<Vec<Option<usize>>>,
) -> Result<Vec<Option<Tile>>, Error> {
