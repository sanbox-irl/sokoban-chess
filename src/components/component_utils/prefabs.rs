use super::{
    serialization_util, Component, ComponentDatabase, Ecs, Entity, PrefabMarker,
    ResourcesDatabase, SerializedEntity, SingletonDatabase,
};

pub fn create_blank_prefab(resources: &mut ResourcesDatabase) -> Result<uuid::Uuid, failure::Error> {
...
pub fn instantiate_prefab_as_child(prefab: &SerializedEntity, ecs: &mut Ecs, parent_id: Entity) -> Entity {
    let entity = instantiate_prefab(prefab, ecs);

    let our_graph_node = ecs.component_database.graph_nodes.get_mut_or_default(&parent_id);
