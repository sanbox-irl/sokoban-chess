...
use gfx_hal::{
    ...
    image::{Layout, Offset, SubresourceRange, Usage},
...
impl<B: Backend> LoadedImage<B> {
    ...
    pub fn edit_image(
        ...
        unsafe {
            ...
            let (staging_bundle, buffer_width) =
                LoadedImage::create_staging_buffer(adapter, device, data, width as usize, height as usize)?;
    ...
    unsafe fn create_staging_buffer(
        ...
        let staging_bundle =
            BufferBundle::new(&adapter, device, upload_size, buffer::Usage::TRANSFER_SRC, false)?;

        let mapping = device
            .map_memory(&staging_bundle.memory, 0..staging_bundle.requirements.size)
            .map_err(|e| LoadedImageError::AcquireMappingWriter(e))?;

        for y in 0..height {
            let data_source_slice = &(*img)[y * row_size..(y + 1) * row_size];
        ...
        Ok((staging_bundle, (row_pitch / std::mem::size_of::<u32>()) as u32))
    }

    unsafe fn load_staging_buffer_into_image_object(
